...
| User's default profile: <br>`profile` | • NHS number: `nhs_number` <br><br> • Surname: `family_name` <br><br> • Date of birth: `birthdate` <br><br> • Identity proofing level: `identity_proofing_level` | No | Yes<span style="vertical-align:super;font-size:60%">1</span> | Yes |
...
| GP registration details: <br>`gp_registration_details`<span style="vertical-align:super;font-size:60%">2</span> | • O.D.S code: `gp_ods_code` | No | Yes | Yes |
| GP surgery information:<br> `gp_integration_credentials` | • Linkage key: `gp_linkage_key` <br><br> • O.D.S. code: `gp_ods_code` <br><br> • Account ID: `gp_user_id` | No | No | Yes<span style="vertical-align:super;font-size:60%">3</span> | 
| Client specific metadata for the user account:<br> `client_metadata`<span style="vertical-align:super;font-size:60%">4</span> | • Client user metadata: `client_user_metadata` | Yes | Yes | Yes |
...
<dt>2. This scope is not required if the 'gp_integration_credentials' scope is requested.</dt>
<dt>3. Available only to IM1 enabled partners and protected by high level of authentication.</dt>
<dt>4. This is a bespoke scope which should only be selected once agreed by NHS login.</dt>
<dl>
